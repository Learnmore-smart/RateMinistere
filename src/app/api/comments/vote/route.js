...
export async function POST(request) {
    ...
    try {
        const { commentId, voteType, teacherId } = await request.json();
        ...
        // Validate input
        if (!commentId || !voteType || !teacherId) {
            return NextResponse.json({
                error: true,
        ...
        if (voteType === 'upvote' && accountId !== existingComment.accountId) {
            try {
                const notificationData = {
                    ...
                    userId: existingComment.accountId,
                    type: 'like',
                    content: {
                        translationKey: 'Notification.CommentLiked',
                        data: {
                ...
                // Continue with the vote response even if notification fails
            }
        }
