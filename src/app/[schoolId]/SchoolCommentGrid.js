            ...
            >
                ...
                <div className={styles['reply-card']}>
                    ...
                    <ReportButton
                        ...
                        type="school-reply"
                        schoolId={schoolId}
                        schoolName={schoolName} // Use schoolName directly
...
const CommentCard = ({
    ...
    onNotLogin,
    schoolId,
    schoolName, // Change this from school to schoolName
    setComments,
    setIsSignUpModalOpen,
    criteria,
    onReport,
                ...
                >
                    <div>
                        <div className={styles['comment-criteria-ratings']}>
                            ...
                            <span className={styles.commentCriteria}>  {/* Display comment criteria */}
                                {comment.commentCriteria || t("SchoolRatingModal.overall")}
                            </span>
                        </div>
                    </div>
                    <ReportButton
                        ...
                        type="school-comment"
                        schoolId={schoolId}
                        schoolName={schoolName} // Use schoolName directly instead of school.name
                        criteriaName={comment.commentCriteria || t("SchoolRatingModal.overall")}
                        setIsSignUpModalOpen={setIsSignUpModalOpen}
            ...
            <div className={styles['replies-container']}>
                {comment.replies && comment.replies.map(reply => (
                    <ReplyCard
                        ...
                        schoolId={schoolId}
                        schoolName={schoolName} // Pass schoolName instead of school object
                        criteria={criteria} // Pass the criteria prop
                        setIsSignUpModalOpen={setIsSignUpModalOpen}
...
const SchoolCommentGrid = ({ comments, onVote, onNotLogin, schoolId, schoolName, setComments, setIsSignUpModalOpen, onReport }) => {  //Recieve all props.
    ...
    useEffect(() => {
        const initializeCommentsWithReplies = async (comments) => {
            ...
            const distributeComments = (commentsWithReplies) => {
                const columns = [[], [], []];
            ...
            distributeComments(commentsWithReplies);
        };

        if (comments?.length) {
            initializeCommentsWithReplies(comments);
        }
                    ...
                    <div key={columnIndex} className={styles['comments-column']}>
                        {column.map((comment) => (
                            <CommentCard
                                ...
                                setIsSignUpModalOpen={setIsSignUpModalOpen}
                                criteria={{
                                    name: comment.commentCriteria || t("SchoolRatingModal.overall")
                                }}
                                onReport={onReport} // Add this
                                commentDetails={commentDetails} // Add this prop
                            />
                        ))}
                    </div>
                ))}
...
export default SchoolCommentGrid;